...
RZ_API const char *rz_bin_dwarf_get_attr_name(ut64 attr_code) {
	if (!((attr_code >= DW_AT_sibling && attr_code <= DW_AT_loclists_base) ||
...
RZ_API const char *rz_bin_dwarf_get_lang_name(ut64 lang) {
	if (lang >= sizeof(dwarf_langs) / sizeof(*dwarf_langs)) {
		return NULL;
	}
...
static RzList /*<RzBinDwarfARangeSet>*/ *parse_aranges_raw(const ut8 *obuf, size_t obuf_sz, PrintfCallback print, bool big_endian) {
	...
	while (buf < buf_end) {
		...
		set->aranges = calloc(set->aranges_count, sizeof(RzBinDwarfARange));
		if (!set->aranges) {
			free(set);
			break;
		}
...
RZ_API RzList /*<RzBinDwarfARangeSet>*/ *rz_bin_dwarf_parse_aranges(RzBinFile *binfile) {
	...
	RzList *r = parse_aranges_raw(buf, len, binfile->rbin->cb_printf, binfile->o && binfile->o->info && binfile->o->info->big_endian);
	free(buf);
	return r;
}
